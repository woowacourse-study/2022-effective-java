...
public class AutoLottoNumberGenerator {
    ...
    private AutoLottoNumberGenerator() {} // 객체 생성 방지
...
public class Lotto {
    ...
    public Lotto(LottoNumberGenerator lottoNumberGenerator) { // 의존 객체 주입
...
이제 특정한 상황으로 (예를들면 로또 번호 6개를 맞춰 1등인 경우)와 같이 외부에서 임의의 숫자를 부여하여 로또를 생성한 뒤 `유연한 테스트`가 가능해진다.
