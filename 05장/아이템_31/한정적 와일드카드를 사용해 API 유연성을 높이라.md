...
이런 코드를 통해 클라이언트 코드는 다음과 같이 사용될 수 있게 된다.
...
정리하자면, 한정적 와일드카드를 제대로만 사용한다면 클래스 사용자는 와일드카드 타입이 쓰였다는 사실조차 의식하지 못할 것입니다.
...
조금 복잡하더라도 와일드카드 타입을 적용하면 API가 훨씬 유연해진다는 것을 알 수 있습니다.
